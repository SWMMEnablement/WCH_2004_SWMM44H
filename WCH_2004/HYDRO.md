...
C=======================================================================
# SWMM Hydro Subroutine – Extensive Summary

This document provides an extensive summary of the Fortran subroutine **HYDRO** used within the SWMM (Storm Water Management Model) code. The subroutine simulates the rainfall-runoff process by performing the following key functions:
...
- **Purpose:**  
      The **HYDRO** subroutine is responsible for computing the water balance in a watershed. It calculates surface runoff, channel flow, groundwater interactions, and continuity errors throughout the simulation period.

- **Historical Update:**  
      *C     Updated 8/93 by Chuck Moore, CDM to include infiltration/inflow*  
      This update added a mechanism to pre-process rainfall data in order to compute infiltration/inflow for each subcatchment.
...
- **Includes and Declarations:**  
      - The subroutine begins by including several common blocks which define constants and parameters (e.g., `TAPES.INC`, `STIMER.INC`, etc.).
      - It declares variables for the hydrological simulation (real numbers, double precision values, logical flags, and character arrays).

- **Initialization:**  
      - Water stored in the channels is computed.
      - Initial conditions such as snowfall flags (`ISNOW`), overland flow (`KWIK`), and channel flow (`KWIKGT`) are set.
      - Precipitation reading is setup with different I/O units.

### 2. Rainfall and Time Step Calculations

- **Reading Precipitation Data:**
      - The subroutine reads the precipitation input file, handling both immediate and future rainfall values.
      - It adjusts the time step based on several conditions:
            - If precipitation (`TRAIN`) is zero or greater than zero.
            - Usage of a WET, DRY, or WETDRY time step depending on the current state of the system.

- **Time Step Adjustments:**  
      - **IMSHORT Flag:**  
            Determines adjustments when precipitation time intervals are shorter than the specified WET duration.
      - **Continuity Initialization:**  
            The subroutine adjusts the simulation step (`NSTEP`) and tracks cumulative simulation time.

### 3. Process Flow through the Simulation

- **Overland Flow Computation:**  
      - The subroutine calls a routine named **WSHED** to compute watershed elements (surface flow).
      - It then handles quality contributions and updates surface runoff by incorporating additional inflows (e.g., infiltration/inflow adjustments).

- **Channel and Pipe Flow:**
      - **GUTTER** is called to simulate channel and pipe routing.
      - The subroutine computes volumes within various channel geometries including trapezoidal, circular, and custom shapes.
      - It also calculates continuity checks by summing inlet and outlet flows.

### 4. Continuity and Error Checks

- **Storage and Continuity:**  
      - The code accumulates various storage components (such as water remaining in storage, infiltration, and snowfall).
      - Continuity errors are computed by comparing total inflows against outflows and storage changes.
      - Detailed formatted outputs provide diagnostic information regarding these balances.

- **Metric Adjustment:**  
      - The results are reported in either English units (inches, cubic feet) or metric units (millimeters, cubic meters) based on the simulation settings.
...
- **Reporting:**  
      - At several stages, the subroutine writes formatted outputs. These include:
            - Precipitation interfaces
            - Continuity checks for surface water, channel/pipe systems, and groundwater
            - Final summary statistics detailing simulation performance and error percentages.

- **Example Format Blocks:**  
      The subroutine uses Fortran FORMAT statements to control the output layout, ensuring clarity when reporting simulation summaries.

---

## Summary of Key Updates (Infiltration/Inflow Feature)

- **Infiltration/Inflow Addition (8/93):**  
      The update by Chuck Moore included enhancements to compute infiltration or inflow at each subcatchment before further routing computations. This ensures a more accurate representation of rain-induced processes by:
      - Adjusting the rainfall values.
      - Distributing rainfall over subareas based on land use fractions.
      - Modifying subsequent water balance calculations accordingly.

---

## Conclusion

The **HYDRO** subroutine is a comprehensive module that integrates rainfall input, water balance computations, and quality checks into a cohesive simulation framework.  
It effectively handles:
- Continuous and non-continuous precipitation events.
- Multiple flow regimes (overland, channel/pipe, and groundwater).
- Both metric and U.S. customary unit systems.

This summary should provide a clear understanding of the structure, functionality, and recent updates made to the HYDRO module to accommodate enhanced infiltration/inflow processing.
